...
run_clippy() {
    ...
    if ! cargo clippy --version >/dev/null 2>&1; then
        print_error "clippy is not available."
        echo
        print_info "Install it with:"
        echo
        printf '  %brustup component add clippy%b\n' "$YELLOW" "$RESET"
        return 1
    fi
    ...
    print_info "Running Clippy on all workspace targets..."
    echo
...
run_clippy 2>&1 | tee "$OUTPUT_FILE"
