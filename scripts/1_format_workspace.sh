...
run_format() {
    echo
    printf '%b\n' "${BOLD}${CYAN}╭─────────────────────────────────────────────╮${RESET}"
    printf '%b\n' "${BOLD}${CYAN}│              FORMAT WORKSPACE               │${RESET}"
    printf '%b\n' "${BOLD}${CYAN}╰─────────────────────────────────────────────╯${RESET}"
    echo
    ...
    if ! cargo fmt --version >/dev/null 2>&1; then
        print_error "rustfmt is not available."
        echo
        print_info "Install it with:"
        echo
        printf '  %brustup component add rustfmt%b\n' "$YELLOW" "$RESET"
        return 1
    fi
    ...
    print_info "Formatting workspace..."
    echo
...
run_format 2>&1 | tee "$OUTPUT_FILE"
